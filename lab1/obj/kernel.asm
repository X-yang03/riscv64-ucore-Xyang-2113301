...
void interrupt_handler(struct trapframe *tf) {
    ...
    80200486:	b8f73723          	sd	a5,-1138(a4) # 80204010 <edata>
                ticks = 0;
    8020048a:	00004797          	auipc	a5,0x4
    8020048e:	b807bb23          	sd	zero,-1130(a5) # 80204020 <ticks>
...
static inline void trap_dispatch(struct trapframe *tf) {
    if ((intptr_t)tf->cause < 0) {  // if the highest bit is 1, then the sighed integer of tf->cause < 0
    80200534:	11853783          	ld	a5,280(a0)
    80200538:	0007c463          	bltz	a5,80200540 <trap+0xc>
        // interrupts
        interrupt_handler(tf);   // highest bit is 1, indicates interrupt
    } else {
        // exceptions
        exception_handler(tf);    // highest bit is 0, indicates exception
